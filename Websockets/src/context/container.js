...
class Container {
    ...
    async saveChat(messages) {
        try {
            // leo el archivo
            const read = await fs.promises.readFile(__dirname+'/files/chat.json', 'utf-8')
            const contents = JSON.parse(read)
            // guardo el nuevo mensaje en el array
            contents.push(messages)
            this.messages = contents
            // guardo el archivo
            await fs.promises.writeFile(__dirname+'/files/chat.json', JSON.stringify(this.messages))
            return messages
        } catch (error) {
            return console.log(`Hubo un error: ${error}`)
        }
